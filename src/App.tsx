...
export default function App() {
  ...
  const openInGoogleMaps = () => {
    if (!orderedStops.length || !startResolved || !finishResolved) return;
  ...
  const buildRoute = async () => {
    try {
      ...
      if (!startResolved) throw new Error("Please enter a start postcode.");
      if (!finishResolved) throw new Error("Please enter a finish postcode.");
      if (!enteredStops.length) throw new Error("Please add at least one job postcode.");
          ...
          <div className="planner-left">
            <div className="panel two-up">
              ...
              <div className="sub-panel">
                ...
                <label>Saved locations</label>
                <select value={finishMode} onChange={(e) => setFinishMode(e.target.value)}>
                  <option value="manual">Manual postcode</option>
                  {savedLocations.map((loc) => (
                    <option key={loc.id} value={loc.id}>
                      {loc.name}
                ...
                <div className="suggested-list">
                  {orderedStops.map((stop) => (
                    <div key={stop.id} className="suggested-item">
                      <div
                        className="travel-pill"
                        style={{ color: getTravelColour(stop.travelMinutesFromPrevious) }}
                      >
